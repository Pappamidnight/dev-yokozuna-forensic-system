...
# -*- coding: utf-8 -*-
"""
ingest_external_drive_i.py - Ingestao Seletiva e Hashing Forense da Unidade Externa I:/
Indexa pecas judiciais, declaracoes IVA/AT, dossie LEA, fotografias WhatsApp e contratos de arrendamento.
"""
import os
import sys
import re
...
PROC_PATTERNS = [
    ...
    (re.compile(r'(spark|venture|filipedelgado|lea|due dilligence)', re.I), "CLUSTER_SOCIETARIO"),
]
...
def detect_cat(filename: str, path_str: str, ext: str) -> Tuple[str, str, str]:
    ...
    if ext in IMG_EXTS:
        if 'whatsapp' in fl or 'img-202' in fl:
    ...
    if 'iva_' in fl or 'finan' in fl or 'comprovativo' in fl or 'cash-flow' in fl or 'recibo' in fl:
        return "FINANCEIRO_COMPROVATIVO", "PROVA_DOCUMENTAL", "ALTA"
        
    if 'requerimento' in fl or 'citacao' in fl or 'oposicao' in fl or 'despacho' in fl or 'sentenca' in fl:
        return "PECA_JUDICIAL_CITIUS", "ATO_PROCESSUAL", "OFICIAL"
        
    if 'contrato' in fl or 'escritura' in fl or 'certidao' in fl or 'palmeira' in fl:
...
def run_ingestion():
    print("=" * 80)
    print(" INGESTÃO FORENSE DA UNIDADE I:/ (ARQUIVO EXTERNO / BACKUP)")
    print(f" Base SQLite : {DB_PATH}")
    print("=" * 80)

    if not I_ROOT.exists():
        print("[AVISO] Unidade I:/ nao esta montada.")
        return
    ...
    for root, dirs, files in os.walk(make_long_path(str(I_ROOT))):
        ...
        for f in files:
            ...
            if sz < 100 or sz > 500 * 1024 * 1024: # Ignorar vazios e ficheiros > 500MB
                continue
            ...
            rel_p = f"Drive_I/{clean_p.replace('I:\\\\', '').replace('I:/', '')}"
            ...
            if len(batch) >= 500:
                ...
                conn.commit()
                batch = []
                sys.stdout.write(f"\r -> Processados de I:/: {total_inserted} registos...")
                sys.stdout.flush()
    ...
    print(f"\n\n{'=' * 80}")
    print(" INGESTÃO DA UNIDADE I:/ CONCLUÍDA")
    print(f"{'=' * 80}")
    ...
    report_file = REPORTS_DIR / "relatorio_ingestao_drive_i.json"
